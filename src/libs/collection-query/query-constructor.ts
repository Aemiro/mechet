              ...
              case FilterOperators.Like: {
                if (aggregateColumns[f.field] === 'jsonb') {
                  langs.forEach((lang, i) => {
                    filterString +=
                      i === 0 && filterIndex === 0
                        ? ` ${columnName}->>'${lang}' like :${f.field}${lang}${paramKey} `
                        : ` OR ${columnName}->>'${lang}' like :${f.field}${lang}${paramKey} `;
                    filterParams[
                  ...
                  filterString +=
                    filterIndex === 0
                      ? ` ${columnName} like :${f.field}${paramKey} `
                      : ` OR ${columnName} like :${f.field}${paramKey} `;
                  filterParams[`${f.field}${paramKey}`] = `%${f.value}%`;
                }
                break;
              }
        ...
        searchFrom.forEach((item: string, index: number) => {
          ...
          if (aggregateColumns[item] === 'jsonb') {
            langs.forEach((lang, i) => {
              searchQuery +=
                i === 0 && index === 0
                  ? ` ${aggregate}.${item}->>'${lang}' like :${paramKey}${lang} `
                  : ` OR ${aggregate}.${item}->>'${lang}' like :${paramKey}${lang} `;
              searchParams[`${paramKey}${lang}`] = `%${search}%`;
            });
          } else {
            searchQuery +=
              index === 0
                ? ` ${aggregate}.${item} like :${paramKey} `
                : ` OR ${aggregate}.${item} like :${paramKey} `;
